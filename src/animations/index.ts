...
// Enhanced parallax animation for sections
const animateParallaxSection = (selector: string, speed: number = 0.5) => {
  gsap.to(selector, {
    yPercent: -50 * speed,
    ease: 'none',
    scrollTrigger: {
...
const animateFloating = (selector: string, duration: number = 3) => {
  gsap.to(selector, {
    y: -20,
    duration: duration,
    ease: 'power1.inOut',
...
const animateScaleOnScroll = (selector: string) => {
  gsap.to(selector, {
    scale: 1.1,
    scrollTrigger: {
      trigger: selector,
...
// Rotate on hover with GSAP
const animateRotateOnHover = (selector: string, rotation: number = 360) => {
